...
対象読者: 日本語環境で 受発注管理システムを利用する一般ユーザー、承認者、管理者、および経費精算を利用する申請者・承認者・精算担当者
...
経費精算は、受発注管理システムとは別のシステムとして扱います。現時点ではマスタ情報管理のロール権限とは連動せず、主に freee 連携状態と freee 側で付与された権限を前提に利用します。
...
初回研修や日常業務では、以下の受発注管理システム向けクイック手順書を利用します。各手順書は、担当者が日常的に使う画面と操作だけをまとめています。
...
受発注管理システムと経費精算は、Agentify にログインした後、「マイアプリ」から起動します。
...
4. 利用するアプリを選択します。
   - 受発注管理システムを利用する場合: 「受注管理」「発注管理」「マスタ情報管理」のいずれかを開きます。
   - 経費精算を利用する場合: 「経費精算」アプリを開きます。利用には freee 連携状態と freee 側の権限を確認してください。
...
- 経費精算で freee 連携や一部操作が利用できない場合は、freee 連携状態または freee 側の権限を確認してください。
...
- 経費精算の利用可否は、マスタ情報管理のロールではなく freee 連携状態と freee 側の権限を前提に記載します。
